...
fn tool_definitions() -> Vec<Value> {
    vec![
        ...
        write_tool(
            "create_project",
            "Create a managed project container in the configured Blackholes_projects folder. With path, independently clone that directory's Git repositories; uncommitted changes must first be committed or stashed. Without path, create a same-named Git repository with an initial commit inside the container. Prepare project-level CLAUDE.md, AGENTS.md, and notes. Original directories remain untouched. Returns the new project metadata.",
            json!({"type":"object","properties":{"name":{"type":"string","minLength":1},"path":{"type":"string","minLength":1}},"anyOf":[{"required":["name"]},{"required":["path"]}]}),
        ),
        write_tool(
            "import_project",
            "Clone local Git repositories into a new managed project container, with independent project instructions, notes, and skills. Committed files only; refuses dirty repositories. Never registers or edits the original directory in place.",
        ...
        write_tool(
            ...
            "Clone a repository into the project's container. Supply either a local path (requires committed changes) or a credential-free GitHub url. A repository already inside the container can be attached directly. Returns project and managed repository metadata; original external checkouts are unchanged.",
