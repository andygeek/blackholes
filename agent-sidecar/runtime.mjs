...
Project repositories can be linked from outside the project root. Resolve the project's root and repository paths through get_project, explicitly read its CLAUDE.md/AGENTS.md and relevant repository instructions before editing, and apply the selected project skills across all attached repositories. Do not rely on parent-directory discovery for external paths or install project instructions into the originals. Direct edits in linked repositories affect the user's original files; task work must still stay in its isolated worktrees.
