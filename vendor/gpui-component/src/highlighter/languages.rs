...
impl Language {
    ...
    pub(super) fn config(&self) -> LanguageConfig {
        ...
        let (language, query, injection, locals) = match self {
            ...
            // `tree-sitter-sequel` pins an older native `cc` toolchain that conflicts with GPUI's
            // current resource compiler. Keep SQL editable as plain text without pulling that
            // incompatible parser into desktop builds.
            Self::Sql => (tree_sitter_json::LANGUAGE, "", "", ""),
