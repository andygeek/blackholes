...
impl RenderOnce for TextViewElement {
    fn render(self, window: &mut Window, cx: &mut App) -> impl IntoElement {
        let is_scrollable = self.list_state.is_some();
        self.state.update(cx, |state, cx| {
            v_flex()
                .w_full()
                .min_w_0()
                .when(is_scrollable, |this| this.h_full())
...
pub(crate) struct TextViewState {
    ...
    last_source: Option<(TextViewType, SharedString)>,
...
impl TextViewState {
    ...
    pub(crate) fn set_multi_click_selection(
        ...
        self.selection_positions = (None, None);
        self.multi_click_selection = Some(TextViewMultiClickSelection { position, kind });
    ...
    pub(crate) fn has_selection(&self) -> bool {
        if self.multi_click_selection.is_some() || self.selected_text_override.is_some() {
            return true;
        }
...
impl TextView {
    ...
    pub fn markdown(
        ...
        let tx = state.update(cx, |state, _| {
            if matches!(
                state.last_source.as_ref(),
                Some((TextViewType::Markdown, text)) if text == &markdown
            ) {
                return None;
            }
            state.last_source = Some((TextViewType::Markdown, markdown.clone()));
            state.tx.clone()
        });
        if let Some(tx) = tx {
    ...
    pub fn html(
        ...
        let tx = state.update(cx, |state, _| {
            if matches!(
                state.last_source.as_ref(),
                Some((TextViewType::Html, text)) if text == &html
            ) {
                return None;
            }
            state.last_source = Some((TextViewType::Html, html.clone()));
            state.tx.clone()
        });
        if let Some(tx) = tx {
...
impl Element for TextView {
    ...
    fn request_layout(
        ...
        }) = self.init_state.take()
        {
            ...
            self.state.update(cx, {
                ...
                |state, _| {
                    ...
                    state.tx = Some(tx);
                    state.last_source = Some((type_, text.clone()));
                }
            });
        ...
        let mut el = div()
            ...
            .w_full()
            .min_w_0()
            .when(self.scrollable, |this| this.h_full())
            .relative()
            .on_action({
