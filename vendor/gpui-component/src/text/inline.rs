...
use crate::{
    ...
    input::Selection,
    text::{TextViewMultiClickKind, node::LinkMark},
};
...
impl Inline {
    ...
    fn layout_selections(
        ...
        if let Some(selection) = text_view_state.multi_click_selection() {
            let selection = selection_for_multi_click(
                &self.text,
                text_layout,
                bounds,
                selection.position,
            ...
            .map(Selection::from);
            return (is_selectable, selection.is_some(), selection);
        }
...
fn selection_for_multi_click(
    ...
    if text.is_empty() || !bounds.contains(&position) {
        return None;
    }
    let offset = text_layout.index_for_position(position).ok()?;
    match kind {
        TextViewMultiClickKind::Word => word_range_at(text, offset),
        TextViewMultiClickKind::Paragraph => Some(0..text.len()),
    }
}
...
fn character_kind(character: char) -> CharacterKind {
...
fn word_range_at(text: &str, offset: usize) -> Option<Range<usize>> {
    let offset = clip_to_char_boundary(text, offset);
    let character = text[offset..].chars().next()?;
    let kind = character_kind(character);
    let mut start = offset;
    let mut end = offset + character.len_utf8();

    for previous in text[..offset].chars().rev().take(128) {
        if character_kind(previous) != kind || kind == CharacterKind::Other {
            break;
        }
        start -= previous.len_utf8();
    }
    for next in text[end..].chars().take(128) {
        if character_kind(next) != kind || kind == CharacterKind::Other {
            break;
        }
        end += next.len_utf8();
    }
...
fn clip_to_char_boundary(text: &str, offset: usize) -> usize {
