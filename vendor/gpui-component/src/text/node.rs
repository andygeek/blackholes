                ...
                .children({
                    ...
                    for (child_ix, child) in children.iter().enumerate() {
                        match child {
                            ...
                            Node::List { .. } => {
                                items.push(div().w_full().min_w_0().ml(rems(1.)).child(
                                    child.render_block(
                                        NodeRenderOptions {
                                            depth: options.depth + 1,
                                            todo: checked.is_some(),
                                            ...
                                            for (ix, cell) in row.children.iter().enumerate() {
                                                ...
                                                cells.push(
                                                    div()
                                                        ...
                                                        // GPUI 0.2.2 mutates text runs while
                                                        // measuring ellipsized text. A later
                                                        // measurement can then reuse the shortened
                                                        // run with the original UTF-8 string and
                                                        // panic when the boundary falls inside a
                                                        // multibyte character. Table cells can wrap,
                                                        // so avoid ellipsis here and keep Unicode
                                                        // content on valid byte boundaries.
                                                        .overflow_hidden()
                                                        .child(
                                                            cell.children
